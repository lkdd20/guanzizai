...
观自在是一个面向古籍与公共领域文本的现代阅读软件。我们希望把安静、清楚的阅读体验与可靠的数字工具结合起来，让读者能够从原文出发，在需要时获得释文、检索和出处辅助。
...
本仓库是经过脱敏的通用源码版本，只附带少量原创、虚构且非宗教的文本用于演示。真实书籍、真实译文、预生成回答、数据库、用户数据、支付信息和密钥均不在仓库中。

自行部署时，请只导入自己拥有版权、已经取得授权，或经当地法律确认可以使用且允许通过互联网提供的非宗教内容。公开版不包含募捐、赞助、收款或支付功能。

公开内容边界和发布检查见 [PUBLIC_CONTENT_POLICY.md](PUBLIC_CONTENT_POLICY.md) 与 [PUBLIC_RELEASE_MANIFEST.md](PUBLIC_RELEASE_MANIFEST.md)。
