...
export const metadata = { title: '创建账户', robots: { index: false, follow: false } }
...
export default async function RegisterPage({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <AuthAccessPanel mode="register" next={next} error={value('error')} provider={value('provider')} sent={value('sent') === '1'} email={value('email')} githubReady={ready && oauthProviderConfigured('github')} googleReady={ready && oauthProviderConfigured('google')} emailReady={ready && smtpConfigured()} />
}
