...
function usage() {
  return [
    ...
    '',
    'Input must be a manually verified work JSON. This script does not fetch, OCR,',
    'or generate original source text.',
  ].join('\n')
}
...
function normalizeImport(rawJson) {
  ...
  const sutra = {
    id: sutraId,
    library: optionalString(sutraInput.library) ?? '国学',
    source_verification:
  ...
  if (sutra.library !== '国学') {
    throw new Error('sutra.library must be "国学" in the public source edition')
  }
...
function buildSql(importData, inputPath) {
  ...
  const statements = [
    ...
    '-- Original source text must be manually verified before this SQL is executed.',
    'BEGIN TRANSACTION;',
